...
<?php $__env->startSection('content'); ?>
    ...
    <?php if($errors->has('errorNumeros')): ?>
        <script>
            $(document).ready(function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: '<?php echo e($errors->first('errorNumeros')); ?>'
                });
            });
        </script>
    <?php endif; ?>
        ...
        function getNumbers() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(item => {
                        let conteudo = item.match(/<a[^>]*>([^<]+)<\/a>/)[1];
                        if (!conteudosVistos[conteudo]) {
                            conteudosVistos[conteudo] = true;
                            novoRaffles.push(item);
                        }
                    })
    ...
    <script>
        ...
        function exibiNovoValorPromocao() {
    ...
    <script type="text/javascript">
        ...
        function numerosAleatorio(valor = null) {
            ...
            const valorSemDesconto = parseFloat(value.replace(',', '.'));
            const qtdSemDesconto = parseInt(qtd);
            const totalSemDesconto = valorSemDesconto * qtdSemDesconto;
            const totalSemDescontoFormatado = totalSemDesconto.toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL'
            });
            ...
            span1.textContent = 'de ' + totalSemDescontoFormatado + ' ';
            span2.textContent = 'por ' + totalFomat.toString().replace(".", ",")

            const listaQtdPromocao = [];
            const container = document.querySelectorAll('#item-promocao-desconto');
            const input = document.querySelector('#numbersA');
            const valueInput = parseInt(input.value);
            const changerTextButton = document.getElementById('numberSelectedTotalHome');

            container.forEach(function(item) {
                const strong = item.querySelector('strong');
                const textStrong = strong.textContent.trim();
                let qtd = textStrong.split(' Por: ')[0];
                let value = textStrong.split(' Por: ')[1];
                listaQtdPromocao.push({
                    "qtd": parseInt(qtd),
                    "value": value
                });
            ...
            listaQtdPromocao.sort(function(a, b) {
                return a - b;
            });

            const ultimoItem = listaQtdPromocao[listaQtdPromocao.length - 1];

            listaQtdPromocao.forEach(function(item) {
                // if (valueInput === item.qtd || valueInput > ultimoItem.qtd) {
                if (valueInput >= item.qtd || valueInput > ultimoItem.qtd) {
                    span1.classList.add('text-decoration-line-through');
                }
            })

            if (qtd <= 1) {
                document.getElementById('numberSelectedTotalHome').innerHTML = '';
                document.getElementById('numberSelectedTotalHome').appendChild(span2);
            } else {
                document.getElementById('numberSelectedTotalHome').innerHTML = '';

                // listaQtdPromocao.forEach(promocao => { 
                // console.log(valueInput===promocao.qtd,ultimoItem.qtd)
                // if (valueInput === promocao.qtd || valueInput >= ultimoItem.qtd) {
                if (valueInput === listaQtdPromocao[0].qtd || valueInput >= ultimoItem.qtd) {
                    document.getElementById('numberSelectedTotalHome').appendChild(span1);
                    span1.classList.add('text-decoration-line-through');
                }
                // })
            ...
            // exibiNovoValorPromocao("<?php echo e($product[0]->price); ?>", qtd)
        }
    </script>
    <script>
        $(document).ready(function() {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Esta função será executada apenas uma vez quando a página for carregada
            addQtd('+');
            addQtd('-');
        });
    </script>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/script42/teste.sistemadirifa.com/resources/views/product-detail.blade.php ENDPATH**/ ?>