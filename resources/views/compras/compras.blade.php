...
<section class="content">
    <div class="row">
        <div class="col-md-12 text-center">
            ...
            <a href="{{ route('product', $rifa->slug) }}" target="_blank"><b><span class="badge bg-success"><i class="fab fa-internet-explorer"></i> Ver Sorteio</b></span></a>
            <a style="cursor: pointer" onclick="liberarReservas('{{ $rifa->id }}')" ><b><span class="badge bg-success"><i class="fas fa-unlock-alt"></i> Liberar Pendentes</span></b></a>
            ...
            <hr>
            <a href="javascript:void(0)"><b><span class="badge bg-primary">{{ $rifa->qtdNumerosDisponiveis() }} Livres </span></b></a>
            <a href="javascript:void(0)"><b><span class="badge bg-warning">{{ $rifa->qtdNumerosReservados() }} Pendentes </span></b></a>
            <a href="javascript:void(0)"><b><span class="badge bg-success">{{ $rifa->qtdNumerosPagos() }} Pagas </span></b></a>
            <hr>
        </div>
    </div>
</section>
            ...
            <form action="{{ route('rifa.comprasBusca', $rifa->id) }}" method="POST">
                <div class="row mb-4 d-none" style="border: 1px solid #000; border-radius: 10px; padding: 5px;"  id="div-search">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" name="search" value="{{ isset($request['search']) ? $request['search'] : '' }}">
                    </div>
                    <div class="col-md-2">
                        <label>Telefone</label>
                        <input type="text" class="form-control" id="telephone" name="telephone" value="{{ isset($request['telephone']) ? $request['telephone'] : '' }}">
                    </div>
                    <div class="col-md-2">
                        <label>Cota</label>
                        <input type="text" class="form-control" name="cota" value="{{ isset($request['cota']) ? $request['cota'] : '' }}">
                    </div>
                    <div class="col-md-2">
                        <label>ID da compra</label>
                        <input type="number" class="form-control" name="idCompra" value="{{ isset($request['idCompra']) ? $request['idCompra'] : '' }}">
                    </div>
                    <div class="col-md-12 mt-2">
                        <button type="submit" class="btn btn-sm btn-success btn-block">BUSACAR PARTICIPANTE</button>
                        <a href="{{ route('rifa.compras', $rifa->id) }}" class="btn btn-sm btn-warning btn-block">LIMPAR BUSCA</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="row">
        ...
        @foreach ($participantes as $participante)
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"><span class="badge bg-success">Participante: {{ $participante->name }}</span></h5>
                        <p class="card-text">
                            ID da Compra: {{ $participante->id }}<br>
                            Total de Cotas: {{ count($participante->numbers()) }}<br>
                            Pagamento Total: R$ {{ number_format($participante->valor, 2, ',', '.') }}<br>
                            Reserva ás: {{ date('d/m/Y H:i:s', strtotime($participante->created_at)) }}<br>
                            Pagamento (PIX) ás: {{ date('d/m/Y H:i:s', strtotime($participante->updated_at)) }}<br>
                            Situação: <span class="badge bg-success">{{ $participante->status() }}</span>
                            <hr>
                        </p>
                        <p class="text-center">
                            <a href="{{ $participante->linkWpp() }}&text=Olá *{{ $participante->name }}.* Agradecemos a sua participação na ação: *{{ $participante->rifa()->name }}*. Suas cotas são: *{{ $participante->numbersResumo() }}*" target="_blank">
                                <b><span class="badge bg-success"><i class="fab fa-whatsapp"></i> Enviar Bilhete Pelo WhatsApp</span></b>
                            </a>
                            <hr>
                        </p>
                        <p class="text-center">
                            <a href="{{ $participante->linkWpp() }}&text=Olá *{{ $participante->name }}.* Agradecemos a sua participação na ação: *{{ $participante->rifa()->name }}*. Gostariamos de anunciar que você foi o ganhador do sorteio!" target="_blank">
                                <b><span class="badge bg-success"><i class="fab fa-whatsapp"></i> Enviar Mensagem de Ganhador</span></b>
                            </a>
                            <hr>
                        </p>
                        <p class="text-center">
                            <a href="{{ route('releaseReservedRafflesNumbers', ['release_reservervations' => $participante->id]) }}" onClick="this.disabled=true; this.innerHTML='Processando...';this.classList.add('disabled')"><b><span class="badge bg-danger"><i class="far fa-trash-alt"></i> Excluir Cota</span></b></a>
                            <a href="{{ route('pagarReservas', ['participante' => $participante->id]) }}"  onClick="this.disabled=true; this.innerHTML='Processando...';this.classList.add('disabled')"><b><span class="badge bg-success"><i class="fas fa-check-double"></i> Aprovar Cota</span></b></a>
                        </p>
                    </div>
                </div>
            </div>
