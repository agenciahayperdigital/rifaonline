...
class CheckoutController extends Controller
{
    ...
    public function pagamentoSucesso(Product $rifa, Participante $participante)
    {
        $responseCotas = $rifa->existeCotaPremiadasNosNumeros($participante->numbers);
        $temCotaPremiada = $responseCotas[1];
        $cotasPremiadas = $responseCotas[0];
        
        // dd($rifa,$participante);
        $rifa;
        $participante;
        $minhas_cotas = json_decode($participante->numbers,true);

        return view('pagamento-sucesso',compact('temCotaPremiada','minhas_cotas','cotasPremiadas','rifa','participante'));
    }
}