...
class ProductAdminController extends Controller
{
    ...
    public function addProduct(Request $request)
    {
        ...
        if (str_starts_with($request->modo_de_jogo, 'fazendinha')) {
            ...
            // if ($request['tipo_cotas'] > 0) {

            //     $codas_premiadas = [];
            //     if ((int)$request['1_n_cota'] >= 0) {
            //         $numberCota = (int)$request['1_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['2_n_cota'] >= 0) {
            //         $numberCota = (int)$request['2_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['3_n_cota'] >= 0) {
            //         $numberCota = (int)$request['3_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['4_n_cota'] >= 0) {
            //         $numberCota = (int)$request['4_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['5_n_cota'] >= 0) {
            //         $numberCota = (int)$request['5_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['6_n_cota'] >= 0) {
            //         $numberCota = (int)$request['6_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['7_n_cota'] >= 0) {
            //         $numberCota = (int)$request['7_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['8_n_cota'] >= 0) {
            //         $numberCota = (int)$request['8_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['9_n_cota'] >= 0) {
            //         $numberCota = (int)$request['9_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            //     };
            //     if ((int)$request['10_n_cota'] >= 0) {
            //         $numberCota = (int)$request['10_n_cota'];
            //         $nbr = str_pad($numberCota, $qtdZeros,  '0', STR_PAD_LEFT);
            //         array_push($codas_premiadas, $nbr);
            //     } else {
            //         array_push($codas_premiadas, 0);
            ...
            //     $rifa->tipo_cotas = $request['tipo_cotas'];
            //     $rifa->cotas_premiadas = $stringCodasPremiadas;
            // }

            $resultado = $rifa->gerarNumeros($qtdNumbers);


            $arr = $resultado['numeros'];
            $tamanhoUltimoNumero = $resultado['ultimoNumero'];
            ...
            $stringCodasPremiadas = $rifa->gerarCodasPremiadas($request->all(), $tamanhoUltimoNumero);
            $rifa->tipo_cotas = $request['tipo_cotas'];
            $rifa->cotas_premiadas = $stringCodasPremiadas;
        ...
        $descricaoProductSorteio = '';

        if ($request->has('description') && $request->description !== '') {
            $descricaoProductSorteio = $request->description;
        } else {
            $descricaoProductSorteio = '';
        }
        ...
        DB::table('product_description')->insert(
            [
                'description' => $descricaoProductSorteio,
                'product_id' => $product,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now()
            ]
        ...
        return redirect()->route('mySweepstakes')->with('success', 'Cadastro da Rifa efetuado com sucesso!');
    }
    ...
    public function adicionaNovoSorteio()
    {
        return view('adiciona-novo-sorteio');
    }

    public function listaTodosSorteios()
    {
        return view('lista-todos-sorteios');
    }
    public function editarUmSorteios()
    {
        return view('editar-um-sorteio');
    }

    public function visualizarSorteio(Request $request, $id = null)
    {
        ...
        return view('visualizar-sorteio');
    }

    public function adicionaNovaFazendinha()
    {
        return view('adiciona-sorteio-fazendinha');
    }
}
