...
class ProductController extends Controller
{
    ...
    public function product($slug, $tokenAfiliado = null)
    {
        ...
        $cotas = DB::table('products')
            ->select('cotas_premiadas', 'tipo_cotas', 'type_raffles')
            ->where('products.id', '=', $productID)
            ->first();
        ...
        $arrayProducts = [
            'cotas' => $cotas, //ok
            'tokenAfiliado' => $tokenAfiliado, //ok
            'imagens' => $imagens, //ok
            'product' => $productDetail, //ok
                ...
                if ($statusProduct->status == "Ativo") {
                    ...
                    if (str_starts_with($prod->modo_de_jogo, 'fazendinha')) {
                        ...
                        if ($prod->type_raffles == 'manual' || $prod->type_raffles == 'mesclado') {
                            ...
                            ###TIRAR NUMEROS COTAS AQUI###
                            // dd($prod->type_raffles,$prod->tipo_cotas,$prod->cotas_premiadas);
                            $disponiveis = $prod->removerCotasPremiadas($disponiveis);

                            // if ($prod->type_raffles === 'automatico') {
                            //     $tipoCotas = intval($prod->tipo_cotas);
                            //     if ($tipoCotas > 0 && $tipoCotas < 100) {
                            //         $porcentagem = $prod->porcentagem();

                            //         if ($porcentagem < $tipoCotas) {

                            //             // $disponivelAntes = count($disponiveis);
                            //             $cotas_escolhidas = explode(',', $prod->cotas_premiadas);
                            //             $disponiveis = array_diff($disponiveis, $cotas_escolhidas);
                            //             // $disponivelDepois = count($disponiveis);
                            //             // echo "<script>alert('mode de rifa -> $prod->type_raffles');</script>";
                            //             // echo "<script>alert('valor das cotas  -> $tipoCotas');</script>";
                            //             // echo "<script>alert('percentual vendido -> $porcentagem percental de cota $tipoCotas');</script>";
                            //             // echo "<script>alert('numero disponiveis -> $disponivelAntes');</script>";
                            //             // echo "<script>alert('numero disponiveis -> $disponivelDepois');</script>";
                            //         }
                            //     }
                            // }
                            ###TIRAR NUMEROS COTAS AQUI###
                    ...
                    $resultadoVerificacaoNumerosIndisponiveis = $prod->existeNumerosIndisponiveis($request->numberSelected);
                    if ($resultadoVerificacaoNumerosIndisponiveis) {
                        return Redirect::back()->withErrors(['errorNumeros' => 'Desculpe, alguns dos números que você escolheu já foram pagos, reservados ou estão indisponíveis. Por favor, escolha números diferentes para prosseguir com a sua seleção.']);
                    }
    ...
    public function gerarPIX(ModelsProduct $product, $resultPricePIX, $email, $name, $participante, $cpf, $telefone)
    {
        ...
        if ($product->gateway == 'mp') {
            ...
            // Adicionando X-Idempotency-Key com um valor único
            $idempotencyKey = uniqid();
            // Adicionando X-Idempotency-Key com um valor único #ALTERACAO#
            \MercadoPago\SDK::addCustomTrackingParam('X-Idempotency-Key', $idempotencyKey);
    ...
    public function todosOsGanhadores()
    {
        $ganhadores = Premio::where('descricao', '!=', null)->where('ganhador', '!=', '')->get();
        ...
        $config = DB::table('consulting_environments')->where('id', '=', 1)->first();
        return view('todos-os-ganhadores', [
            'products' => $products,
            'winners' => $winners,
            'ganhadores' => $ganhadores,
            'user' => User::find(1),
    ...
    public function termosDeUsoEParticipacao()
    {
        ...
        $config = DB::table('consulting_environments')->where('id', '=', 1)->first();
        return view('termos-de-uso-e-participacao', [
            'products' => $products,
            'winners' => $winners,
            'ganhadores' => $ganhadores,
            'user' => User::find(1),
