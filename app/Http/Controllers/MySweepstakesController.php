...
class MySweepstakesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $rifas = ModelsProduct::orderBy('id', 'desc')->paginate(4);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $rifa_edit = Product::find($id);



            $numeros_rifa = $rifa->numbers();
            $ultimoNumero = end($numeros_rifa);
            $tamanhoUltimoNumero = strlen($ultimoNumero);


            #FIM TESTE DE UPDATE DE COTAS PREMIADAS#
            ####EM TESTE####
        ...
        if ($prod->promocoes()->count() === 0) {
            ...
            $limit = count($request->numPromocao);
            if ($limit > 1) {
                foreach ($request->numPromocao as $ordem => $promocao) {
                    $qtdNumeros = $request->numPromocao[$ordem];
                    $desconto = floatval($request->valPromocao[$ordem]);
                    $total = $qtdNumeros * str_replace(",", ".", $prod->price);
                    $valorComDesconto = $total - ($total * $desconto / 100);

                    Promocao::where('product_id', '=', $id)->where('ordem', '=', $ordem)->update([
    ...
    // ##CRIADA FUNCAO  PARA LISTAGEM DE CLIENTES
    public function clientes()
    {
        $clientes = DB::table('customers')->paginate(20);
        return view('clientes', ['clientes' => $clientes]);
    }
    // ##CRIADA FUNCAO  PARA EDITAR UM CLIENTE

    public function gateway_pagamento()
    {
        ...
        return view('gateway-pagamento', [
            'users' => $users
        ]);
    }

    public function update_gateway_pagamento(Request $request)
    {
        ...
        if ($request->type_gateway === 'paggue') {
    ...
    public function pixel_google_ads()
    {


        $ads_environments = DB::table('ads_environments')
            ->select('cod_verif_dom_facebook', 'cod_header_facebook', 'cod_body_google', 'cod_header_google', 'PiexelPageViwer', 'PiexelAddToCart', 'PiexelPurchase')
            ->join('users', 'users.id', '=', 'ads_environments.user_id')
            ->where('users.id', '=', Auth::user()->id)
            ->first();

        return view('pixel-google-ads', [
            'ads' => $ads_environments
        ]);
    }

    public function update_pixel_google_ads(Request $request)
    {

        if ($request->type_ads === 'pixel') {
            $consulting = DB::table('ads_environments')
                ->where('ads_environments.user_id', Auth::user()->id)
                ->update(
                    [
                        'PiexelPageViwer' => $request->PiexelPageViwer,
                        'PiexelAddToCart' => $request->PiexelAddToCart,
                        'PiexelPurchase' => $request->PiexelPurchase,
                        // 'cod_verif_dom_facebook' => $request->cod_verif_dom_facebook,
                        // 'cod_header_facebook' => $request->cod_header_facebook,
                    ]
                );
        }

        if ($request->type_ads === 'google') {
            $consulting = DB::table('ads_environments')
                ->where('ads_environments.user_id', Auth::user()->id)
                ->update(
                    [
                        'cod_header_google' => $request->cod_header_google,
                        'cod_body_google' => $request->cod_body_google,
                    ]
                );
        }
    ...
    public function editarCliente(Request $request)
    {
        ...
        $request->validate([
            'editInputPassword' => 'required',
            'editInputCode' => 'required',
        ]);

        $telefone = $request->input('editInputPassword');
        $id = $request->input('editInputCode');

        $affected = DB::table('customers')
            ->where('id', '=', intval($id))
            ->update(['telephone' => $telefone]);

        $bilhetes = DB::table('participant')
    ...
    // ##CRIADA FUNCAO  PARA EXCLUIR UM CLIENTE
    public function excluirCliente(Request $request)
    {

        $request->validate([
            'excluirInputCode' => 'required'
        ]);

        $id = $request->input('excluirInputCode');
    ...
    public function resumoRifa($id)
    {
        ...
        if (!$rifa) {
            return redirect()->route('mySweepstakes');
        }


        $config = DB::table('consulting_environments')->where('id', '=', 1)->first();

        $participantes = DB::table('participant')->where('product_id', '=', $rifa->id)->paginate(20);
        // dd($rifa->id,$participantes);
        $data = [
            'rifa' => $rifa,
            'config' => $config,
            'participantes' => $participantes,
    ...
    public function caixaDeMenssagens()
    {

        return view('caixa-de-menssagens');
    }
}